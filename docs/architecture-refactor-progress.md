...
| Module | Before | Actual responsibility | After | Reason | Tests | Remaining risk |
| --- | --- | --- | --- | --- | --- | --- |
| auth | Shared package provides transport contracts, failures, and presentation events while apps own role-specific repositories | The shared code is still feature-level rather than provider-only | Duplicate the small auth workflows inside each app, then remove `packages/auth` | Driver and Passenger auth must evolve independently | Targeted validation pending | Preserve secure session behavior and failure copy while removing the package |
...
- remove `shared_core`, `packages/auth`, `packages/chat`, and `packages/ride` after reference and export audits
