...
class _SearchDestinationScreenState extends State<SearchDestinationScreen> {
  ...
  Future<void> _loadNearbyPlaces() async {
    ...
    final queries = ["plaza", "mall", "market", "restaurant", "school", "hospital"];
    List<PlaceModel> all = [];

    for (final q in queries.take(3)) {
      final results = await MapProvider.searchPlaces(q, lat: _userLat, lng: _userLng);
  ...
  Future<void> _performSearch() async {
    ...
    final results = await MapProvider.searchPlaces(query, lat: _userLat, lng: _userLng);
    if (mounted) {
      setState(() {
        _results = results;
        _isSearching = false;
          ...
          child: Hero(
            ...
            child: Material(
              ...
              child: Container(
                ...
                child: Row(
                  children: [
                    Icon(LucideIcons.search, color: AppTheme.primaryColor.withValues(alpha: 0.6), size: 18),
                    const SizedBox(width: 10),
                    Expanded(
                      child: TextField(
                        controller: _searchController,
                        autofocus: true,
                        style: const TextStyle(fontSize: 15, color: AppTheme.primaryColor),
                        decoration: InputDecoration(
                          hintText: "Search destination",
                          hintStyle: TextStyle(fontSize: 15, color: AppTheme.primaryColor.withValues(alpha: 0.6), fontWeight: FontWeight.w400),
                    ...
                    if (_searchController.text.isNotEmpty)
                      GestureDetector(
                        ...
                        child: Icon(LucideIcons.x, size: 16, color: AppTheme.primaryColor.withValues(alpha: 0.5)),
                      ),
                    Container(height: 24, width: 1, margin: const EdgeInsets.symmetric(horizontal: 8),
                      color: AppTheme.primaryColor.withValues(alpha: 0.1)),
                    GestureDetector(
                      onTap: _openMapPin,
                      child: Row(mainAxisSize: MainAxisSize.min, children: [
                        const Icon(LucideIcons.map_pin, color: AppTheme.primaryColor, size: 18),
                ...
                : displayList.isEmpty
                    ? Center(
                        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                          Icon(hasQuery ? LucideIcons.search_x : LucideIcons.map_pin_off, size: 40,
                            color: AppTheme.primaryColor.withValues(alpha: 0.2)),
                          const SizedBox(height: 12),
                          Text(hasQuery ? "No places found" : "Locating nearby places...",
                            style: TextStyle(color: AppTheme.primaryColor.withValues(alpha: 0.4), fontWeight: FontWeight.w600)),
                        ]),
                      )
                    : ListView.separated(
  ...
  Widget _buildPlaceTile(PlaceModel place) {
    return ListTile(
      ...
      title: Text(place.name,
        style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 15, color: AppTheme.primaryColor)),
      subtitle: Text(place.fullAddress,
        style: TextStyle(color: AppTheme.primaryColor.withValues(alpha: 0.5), fontSize: 12),
      ...
      trailing: place.distanceKm != null
          ? Container(
              ...
              child: Text("${place.distanceKm!.toStringAsFixed(1)} km",
                style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: AppTheme.tertiaryColor)),
            )
          : const Icon(LucideIcons.chevron_right, size: 16, color: AppTheme.borderSide),
