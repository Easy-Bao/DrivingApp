...
export class PassengerService {
  ...
  async registerEmail(payload: { email: string; password: string }) {
    const passwordHash = await Bun.password.hash(payload.password, {
      algorithm: 'bcrypt',
      cost: 10,
    });

    const passenger = await this.repository.registerOrUpdateEmail(payload.email, passwordHash);
    const otpCode = Math.floor(DEFAULT_OTP_MIN + Math.random() * DEFAULT_OTP_RANGE).toString();
    otpsMap.set(payload.email, { code: otpCode, expires: Date.now() + OTP_EXPIRY_MS });
  ...
  async verifyEmailOtp(payload: { email: string; code: string }) {
  ...
  async completeProfile(passengerId: string, payload: { name: string; phone: string }) {
    const updated = await this.repository.completePassengerProfile(passengerId, payload.name, payload.phone);
    const { password_hash, ...passengerWithoutPassword } = updated as any;
    return passengerWithoutPassword;
  }
}
