...
describe('Passenger Service Integration Tests', () => {
  ...
  describe('Email-First Onboarding Flow API Tests', () => {
    const testEmail = 'onboarding_passenger@example.com';
    let onboardingToken = '';

    test('POST /passengers/register-email - creates a pending passenger', async () => {
      const response = await app.request('/passengers/register-email', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          email: testEmail,
    ...
    test('POST /passengers/verify-email-otp - verifies code and returns JWT', async () => {
      const response = await app.request('/passengers/verify-email-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          email: testEmail,
      ...
      const data: any = await response.json();
      expect(data.token).toBeDefined();
      expect(data.passenger).toBeDefined();
      expect(data.passenger.email).toBe(testEmail);
      expect(data.passenger.is_verified).toBe(true);

      onboardingToken = data.token;
    });

    test('PUT /passengers/complete-profile - updates name and phone securely', async () => {
      const response = await app.request('/passengers/complete-profile', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
      ...
      expect(response.status).toBe(200);
      const data: any = await response.json();
      expect(data.name).toBe('Onboarded User');
      expect(data.phone).toBe('+639999999999');
