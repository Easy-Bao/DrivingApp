...
func TestPassengerOTPVerifiesAndConsumesCode(t *testing.T) {
	...
	gateway := &otpGateway{}
	service := usecase.NewOTPService(repository, store, gateway, otpIssuer{})
...
func TestDriverCannotUsePassengerVerificationOTP(t *testing.T) {
	...
	service := usecase.NewOTPService(repository, &otpMemoryStore{values: map[string]string{}}, &otpGateway{}, otpIssuer{})
