...
func (repository *PostgresDashboardStatsRepository) Stats(ctx context.Context) (domain.Stats, error) {
	...
	userCount, err := postgresCountToInt(users)
	if err != nil {
		return domain.Stats{}, fmt.Errorf("map user count: %w", err)
	}
	rideCount, err := postgresCountToInt(rides)
	if err != nil {
		return domain.Stats{}, fmt.Errorf("map ride count: %w", err)
	}
	documentCount, err := postgresCountToInt(documents)
	if err != nil {
...
func postgresCountToInt(value int64) (int, error) {
	if value < 0 {
