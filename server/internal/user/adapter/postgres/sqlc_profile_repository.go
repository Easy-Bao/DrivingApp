...
func (repository *PostgresProfileRepository) Get(ctx context.Context, userID int) (domain.Profile, error) {
	...
	postgresUserID, err := toPostgresProfileID(userID, "user id")
	if err != nil {
		return domain.Profile{}, err
	}

	account, err := repository.queries.GetUserByID(ctx, postgresUserID)
	if err != nil {
		return domain.Profile{}, fmt.Errorf("find profile account: %w", err)
	}

	driverProfile, err := repository.queries.GetDriverProfileByUserIDFull(ctx, postgresUserID)
	if err == nil {
	...
	passengerProfile, err := repository.queries.GetPassengerProfileByUserIDFull(ctx, postgresUserID)
	if err != nil {
...
func (repository *PostgresProfileRepository) Save(ctx context.Context, profile domain.Profile) (domain.Profile, error) {
	...
	postgresUserID, err := toPostgresProfileID(profile.UserID, "user id")
	if err != nil {
		return domain.Profile{}, err
	}
	postgresProfileID, err := toPostgresProfileID(profile.ID, "profile id")
	if err != nil {
		return domain.Profile{}, err
	}
...
func (repository *PostgresProfileRepository) SaveAvatar(
	...
	oldStorageKey := strings.TrimSpace(postgresProfileTextValue(passengerProfile.AvatarStorageKey))
...
func (repository *PostgresProfileRepository) GetAvatar(ctx context.Context, userID int) (domain.Avatar, error) {
	...
	storageKey := strings.TrimSpace(postgresProfileTextValue(passengerProfile.AvatarStorageKey))
	if storageKey == "" {
		return domain.Avatar{}, domain.ErrAvatarNotFound
	}
	...
	storedContentType := strings.TrimSpace(postgresProfileTextValue(passengerProfile.AvatarContentType))
...
func (repository *PostgresProfileRepository) Notifications(
	...
	postgresLimit, err := toPostgresProfilePageValue(limit+1, "limit")
	if err != nil {
		return nil, err
	}
	postgresOffset, err := toPostgresProfilePageValue(offset, "offset")
	if err != nil {
		return nil, err
	}
...
func (repository *PostgresProfileRepository) DeleteNotification(
	...
	postgresNotificationID, err := toPostgresProfileID(notificationID, "notification id")
	if err != nil {
		return err
	}
	deleted, err := repository.queries.DeleteNotification(ctx, databasepostgres.DeleteNotificationParams{
		ID:     postgresNotificationID,
		UserID: postgresUserID,
	})
	if err != nil {
...
func passengerProfileFromPostgres(account databasepostgres.User, profile databasepostgres.PassengerProfile) domain.Profile {
	return domain.Profile{
		...
		Phone:             account.Phone,
		Email:             account.Email,
		Address:           postgresProfileTextValue(profile.Address),
		Gender:            profile.Gender,
		AvatarURL:         passengerAvatarURL(int(profile.UserID), postgresProfileTextValue(profile.AvatarStorageKey)),
		PreferredRideType: postgresProfileTextValue(profile.PreferredRideType),
	}
}
...
func postgresProfileTextValue(value pgtype.Text) string {
	if !value.Valid {
		return ""
	}
	return value.String
