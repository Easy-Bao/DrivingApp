...
describe('Trip Service', () => {
  ...
  test('GET /rides/active — returns pending ride requests', async () => {
    const res = await tripApp.fetch(new Request('http://localhost/rides/active'));
  ...
  test('GET /rides/:id — retrieves a specific ride', async () => {
    const res = await tripApp.fetch(new Request(`http://localhost/rides/${rideId}`));
    expect(res.status).toBe(200);
    const data: any = await res.json();
  ...
  test('POST /rides/:id/accept — assigns driver to ride', async () => {
    const res = await tripApp.fetch(
      new Request(`http://localhost/rides/${rideId}/accept`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  test('POST /rides/:id/status — transitions through arrived and in_transit', async () => {
    for (const status of ['arrived', 'in_transit']) {
      const res = await tripApp.fetch(
        new Request(`http://localhost/rides/${rideId}/status`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  test('POST /rides/:id/status — completed removes from active list', async () => {
    ...
    expect(res.status).toBe(200);
    const activeRes = await tripApp.fetch(new Request('http://localhost/rides/active'));
