...
/// Centralized environment and service configuration.
///
...
class EnvironmentConfig {
  ...
  /// Resolves and builds the base passenger service URI from the environment.
  static Uri get passengerServiceUri {
    final rawUrl = dotenv.env['PASSENGER_SERVICE_URL'];
    if (rawUrl == null || rawUrl.isEmpty) {
      throw StateError(
        'PASSENGER_SERVICE_URL environment variable is missing or empty.',
      );
    }
    return _resolveUri(rawUrl);
  }

  /// Resolves and builds the base trip service URI from the environment.
  static Uri get tripServiceUri {
    final rawUrl =
        dotenv.env['TRIP_SERVICE_URL'] ?? dotenv.env['PASSENGER_SERVICE_URL'];
    if (rawUrl == null || rawUrl.isEmpty) {
      throw StateError(
        'TRIP_SERVICE_URL environment variable is missing or empty.',
      );
    }
    return _resolveUri(rawUrl);
  }
